...
class PbxUser(models.Model):
    ...
    def fagi_request(self, request):
        ...
        extension_plus = '+{}'.format(extension) if not extension.startswith('+') else extension
        users = self.env['asterisk_plus.user'].search(
            ['|', ('phone_normalized', '=', extension_plus), ('exten', '=', extension)])
        endpoints = []
        if len(users) == 1:
            user = users[0]
            ...
            if user.record_calls:
                res.append('EXEC MIXMONITOR {}.wav'.format(request['agi_uniqueid']))
